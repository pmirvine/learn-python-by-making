"""The pantry, with its two faults put right: the data, and the try block."""
...
ITEMS = {
    ...
    "jam": Item("Home-made strawberry jam, with a gingham hat on.", 2),
...
def take(
    ...
    # Only one line here can fail in the way we expect, so only that line is
    # in the try. A KeyError from anywhere else is a bug, and should be seen.
    try:
        where = places[name]
    except KeyError:
