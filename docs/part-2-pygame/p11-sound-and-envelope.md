...
You have graphics, sound, and two games. Next comes a game that needs some mathematics. In Project 12 a spaceship drifts, turns and fires, and rocks break into smaller rocks. It's Asteroids, and you can't write it without *vectors*. Python lets you make a type of your own that adds with `+`, scales with `*` and compares with `==`, as if it had always been part of the language.
