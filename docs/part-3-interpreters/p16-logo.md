...
Logo's programs are lists of words, read again each time they're run. The languages that most people learned on a BBC Micro had *line numbers*, and `GOTO`, and an interpreter in 16K of ROM that was among the fastest of its day. In [Project 17](p17-tiny-basic.md) you'll write BASIC, and this time you'll parse the program into a tree of dataclasses, walk it with `match`, and teach Pylance to check the whole thing in its strictest mode.
