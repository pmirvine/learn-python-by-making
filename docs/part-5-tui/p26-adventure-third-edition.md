...
That's the end of Part 5, and of the building. You've made games, interpreters, web sites and terminal applications, and nearly all of them are sitting in a folder on your own computer, where nobody else can use them. Part 6 is about the last step, which is a craft of its own: versions, changelogs, builds, releases, and a command that anybody in the world can install. It's time to ship.
